                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('register') }}">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                @for ($i = 0; $i < 3; $i++)
                                    <input type="text" class="form-control mb-2 @error('hobbies.' . $i) is-invalid @enderror" name="hobbies[]" placeholder="Hobby {{ $i + 1 }}" value="{{ old('hobbies.' . $i) }}" required>
                                    @error('hobbies.' . $i)
                                        <span class="invalid-feedback" role="alert"><strong>{{ $message }}</strong></span>
                                    @enderror
                                @endfor
                        ...
                        <!-- Price -->
                        <div class="row mb-3">
                            <label class="col-md-4 col-form-label text-md-end">{{ __('Registration Price') }}</label>
                            <div class="col-md-6">
                                <div class="alert alert-info" role="alert">
                                    <strong>Price:</strong> Rp {{ number_format($price, 0, ',', '.') }}
                                </div>
                            </div>
                        </div>
